...
func (f *Fetch) UserFetchRepositoryDiscussionCommentsReactionsMore(node types.StargazersNode, respData types.GraphqlQueryUserResponse, userInfo UserInfo) {
	for ix, discussionCommentsNode := range node.RepositoryDiscussionComments.Nodes {
		...
		var score int
		var upvoteScore int
		if respData.User.RepositoryDiscussionComments.Nodes[ix].ViewerHasUpvoted {
			upvoteScore = respData.User.RepositoryDiscussionComments.Nodes[ix].UpvoteCount - 1
		} else {
			if respData.User.RepositoryDiscussionComments.Nodes[ix].UpvoteCount > 1 {
				upvoteScore = respData.User.RepositoryDiscussionComments.Nodes[ix].UpvoteCount
			}
		}
		temp := CalculateScoreReactions(respData.User.RepositoryDiscussionComments.Nodes[ix].Reactions.Nodes, strings.Split(userInfo.FullName, "/")[0])
		score += temp + upvoteScore
		for running {
			req := graphql.NewRequest(userQuery.RepositoryDiscussionCommentsReactionsMore)
			// set any variables
			...
			temp := CalculateScoreReactions(respData.User.RepositoryDiscussionComments.Nodes[ix].Reactions.Nodes, strings.Split(userInfo.FullName, "/")[0])
			score += temp + upvoteScore
			running = respData.User.RepositoryDiscussionComments.Nodes[ix].Reactions.PageInfo.HasNextPage
			if running {
				fetchMore = respData.User.RepositoryDiscussionComments.Nodes[ix].Reactions.PageInfo.EndCursor
			}
		}
