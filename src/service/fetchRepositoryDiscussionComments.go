...
func (f *Fetch) FetchRepositoryDiscussionComments(node types.StargazersNode, idx int, wg *sync.WaitGroup, userInfo UserInfo) {
	...
	for running {
		wg.Add(1)
		req := graphql.NewRequest(stargazersQuery.RepositoryDiscussionComments)
		// set any variables
		...
		f.FetchRepositoryDiscussionCommentsReactionsMore(respData.Repository.Stargazers.Nodes[idx], idx, wg, respData, userInfo)
		running = respData.Repository.Stargazers.Nodes[idx].RepositoryDiscussionComments.PageInfo.HasNextPage
		if running {
			fetchMore = respData.Repository.Stargazers.Nodes[idx].RepositoryDiscussionComments.PageInfo.EndCursor
		}
	}
	wg.Done()
}
